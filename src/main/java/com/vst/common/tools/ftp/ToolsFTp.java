...
public class ToolsFTp {
	 ...
	 * @param remoteHost
	 * @param fileStream
	 * @param fileName 文件名称，不包含.后缀名
	 * @param fileType 文件类型
	 * @return
	 */
	public String uploadFile(String destDir, String remoteHost, InputStream fileStream, String fileName, String fileType){
		// 返回标志
		String result = null;
		// 输入流
		FileInputStream fis = null;
		...
		try {
			...
			if(!ToolsString.isEmpty(destDir)){
				...
				if(!FTPReply.isPositiveCompletion(reply)){
					...
					String fileName2 = fileName + fileType;
					if(ftp != null){
						ftp.makeDirectory(destDir);
						ftp.changeWorkingDirectory(destDir);
						boolean flag = ftp.appendFile(fileName2, fileStream);
						// 如果上传成功，才返回路径
						if(flag) result = destDir + "/" + fileName2;
						logger.info("upload file "+ flag +"!" + result);
					}else{
						logger.info("upload file error!");
					}
